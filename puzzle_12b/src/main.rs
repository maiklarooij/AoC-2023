fn main() {
    ...
    for line in lines {
        ...
        springs.push('?');
        let mut repeated_springs = springs.clone().into_iter().cycle().take(springs.len() * 5).collect::<Vec<char>>();
        let len_springs: usize = repeated_springs.len()-1;
        repeated_springs[len_springs] = '.';
        ...
        println!("Springs: {:?}", repeated_springs);
        println!("Spring sizes: {:?}", spring_sizes);
        
        // Find all valid arrangements
        let valid_arrangements_springs: u32 = valid_arrangements(repeated_springs, spring_sizes);
